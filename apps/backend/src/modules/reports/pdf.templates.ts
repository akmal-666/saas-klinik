 ...
 * 
 * Strategi: Generate HTML → Convert ke PDF menggunakan Puppeteer (server) 
 * atau @react-pdf/renderer (in-memory, lebih ringan).
 *
 * Untuk Vercel/Railway (memory-constrained):
 *   Gunakan api route yang memanggil endpoint PDF secara on-demand.
 *   PDFs di-cache di Supabase Storage setelah pertama kali dibuat.
 */
...
export function buildInvoiceHtml(invoice: {
  ...
  created_at: string;
  clinic: { name: string; branch: string; address: string; phone: string; };
  patient: { full_name: string; rm_number: string; phone: string; };
  items: { treatment_name: string; quantity: number; unit_price: number; total: number; }[];
  subtotal: number;
...
export function buildPayrollSlipHtml(slip: {
  doctor_name: string;
  period: string;
  share_code: string;
  doctor_pct: number;
  ...
  total_visits: number;
  clinic: { name: string; branch: string; };
  generated_at: string;
}): string {
  const fmt = (n: number) => 'Rp ' + n.toLocaleString('id-ID');
